[<template>
  <div class="container p-2">
    ...
    <div v-if="!this.loading">
      <h3>
        Current state:
        {{ this.$props["has_private_token"] === true ? "Private" : "Public" }}
      </h3>
      <div>
        <FormulateForm
          v-model="formValues"
        ...
        >
          <div
            v-if="
              this.$props['has_private_token'] && formValues.has_private_token
            "
          >
            <hr />
          ...
          <FormulateErrors />
          <FormulateInput label="Change accessibility" type="submit" />
        </FormulateForm>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      const emailValidation = this.validateNewEmails(request.newInvitedEmails);
      if (!emailValidation) {
        await Form.putUpdateAccessForm(request, this.$props["slug"]).then(
          (res) => {
            try {
              if (res.status === 200) {
                 this.$toasted.success(`Changing accessibility was successful.`);
                 setTimeout(()=>{
                ...
                this.$toasted.error(`Changing accessibility failed (Error: ${error})`);
                this.loading = false;
            }
          }
        );
